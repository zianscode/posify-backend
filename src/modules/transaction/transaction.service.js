...
class TransactionService {
     ...
     */
    async createTransaction(userId, outletId, data) {
        // 1. Verify user is assigned to an outlet
        if (!outletId) {
            throw new errors_1.BadRequestError("Pengguna tidak dikaitkan dengan outlet mana pun");
        }
        ...
        const addonsObjList = allAddonIds.length > 0 ? await database_1.prisma.addOn.findMany({
        ...
        const itemsData = items.map((item, idx) => {
            ...
            if (!product) {
                throw new errors_1.BadRequestError(`Produk dengan ID ${item.productId} tidak ditemukan`);
            }
            ...
            for (const addonItem of resolvedAddons) {
                ...
                if (!addonObj) {
                    throw new errors_1.BadRequestError(`Add-on dengan ID ${addonItem.id} tidak ditemukan`);
                }
        ...
        if (paidAmount < grandTotal) {
            throw new errors_1.BadRequestError(`Pembayaran kurang dari total belanja. Total: ${grandTotal}, Dibayar: ${paidAmount}`);
        }
        // 4. Atomically insert transaction, items, update stocks and write movements logs
        const transactionId = await database_1.prisma.$transaction(async (tx) => {
            // Verify payment method exists
            const paymentMethod = await tx.paymentMethod.findUnique({
                where: { id: paymentMethodId },
            });
            if (!paymentMethod) {
                throw new errors_1.BadRequestError("Metode pembayaran tidak ditemukan");
            }
            ...
            for (const item of itemsData) {
                ...
                if (currentProduct.stock < item.quantity) {
                    throw new errors_1.BadRequestError(`Stok produk "${currentProduct.name}" tidak mencukupi. Stok saat ini: ${currentProduct.stock}, diminta: ${item.quantity}`);
                }
            }
        ...
        return {
            ...
            items: result.items.map((item) => ({
                ...item,
                addons: flattenAddons(item.addons),
            })),
        };
    }
    async notifyLowStock(outletId, itemsData) {
